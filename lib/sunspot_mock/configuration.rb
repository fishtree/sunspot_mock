...
module Sunspot #:nodoc:
  module Mock #:nodoc:
    ...
    class Configuration
      ...
      def pid_dir
        @pid_dir ||= user_configuration_from_key('solr', 'pid_dir') || File.join(Dir.pwd, 'solr', 'pids', ENV['RACK_ENV'])
      end
      ...
      def default_log_file_location
        File.join(Dir.pwd, 'log', "solr_" + ENV['RACK_ENV'] + ".log")
      end
      ...
      def user_configuration
        @user_configuration ||=
          begin
            path = File.join(Dir.pwd, 'config', 'sunspot.yml')
            if File.exist?(path)
              File.open(path) do |file|
                processed = ERB.new(file.read).result
                YAML.load(processed)[ENV['RACK_ENV']]
              end
            else
              {}
            end
